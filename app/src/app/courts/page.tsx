    ...
    <div className="space-y-8">
      ...
      <div className="flex gap-3 items-end">
        <div>
          ...
          <select value={city} onChange={(e) => setCity(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 text-gray-900">
        ...
        <button onClick={scrapeCourts} disabled={scraping} className="bg-green-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-green-700 transition-colors disabled:opacity-50">
          {scraping ? "Scraping..." : "Scrape Courts"}
        </button>
      </div>
      ...
      {!loading && courts.length === 0 && (
        <p className="text-gray-500">No courts found for {city}. Click &quot;Scrape Courts&quot; to discover available courts.</p>
      )}
    </div>
  );
}
